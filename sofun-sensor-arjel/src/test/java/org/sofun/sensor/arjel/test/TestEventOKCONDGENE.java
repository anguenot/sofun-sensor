...
/**
 * Test event OKCONDGENE XML generation.
 * 
 * @author <a href="mailto:julien@anguenot.org">Julien Anguenot</a>
 * 
 */
public class TestEventOKCONDGENE extends AbstractARJELTestCase {
    ...
    public void testValidEvent() throws Exception {
        ...
        EventOKCONDGENE trace = new EventOKCONDGENEImpl(operatorID, safeID,
                eventID, eventDate, playerID, playerIP, sessionID, fieldTypAg);

        trace.validate();

        String xml = trace.getXML();
        assertEquals(getXMLFrom("event/OKCONDGENE.xml"), xml);

        xml = trace.translate();
        assertEquals(getRootXMLFrom("event/OKCONDGENE_TR.xml"), xml);
