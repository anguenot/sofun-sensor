...
/**
 * Test event LOTNATURE XML generation.
 * 
 * @author <a href="mailto:julien@anguenot.org">Julien Anguenot</a>
 * 
 */
public class TestEventLOTNATURE extends AbstractARJELTestCase {
    ...
    public void testValidEvent() throws Exception {
        ...
        final String fieldLotNNom = "Air Jordan 4 retro";

        final String fieldLotNValeur = "140";

        XMLTrace trace = new EventLOTNATUREImpl(operatorID, safeID, eventId,
                eventDate, playerID, playerIP, sessionID, fieldTypAg,
                fieldLotNNom, fieldLotNValeur);

        trace.validate();

        String xml = trace.getXML();
        assertEquals(getXMLFrom("event/LOTNATURE.xml"), xml);

        xml = trace.translate();
        assertEquals(getRootXMLFrom("event/LOTNATURE_TR.xml"), xml);
